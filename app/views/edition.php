<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>map edition</title>
    <link rel="shortcut icon" href="<?= Flight::base() ?>/public/assets/compiled/svg/favicon.svg" type="image/x-icon">
    <link rel="shortcut icon" href="data:image/png;base64," type="image/png">
    <link rel="stylesheet" href="<?= Flight::base() ?>/public/assets/compiled/css/app.css">
    <link rel="stylesheet" href="<?= Flight::base() ?>/public/assets/compiled/css/app-dark.css">

    <!-- modul css -->    
    <link rel="stylesheet" href="<?= Flight::base() ?>/public/assets/compiled/css/iconly.css">
    <link rel="stylesheet" href="<?= Flight::base() ?>/public/assets/extensions/flatpickr/flatpickr.min.css">

    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
</head>
<style>
    #main {
        flex: 1 1 auto;       
        padding: 0 !important;
    }

    #map {
        width: 100%;
        height: 100vh;
    }

    .commune-label {
        font-size: 12px;
        font-weight: bold;
        color: darkblue;
        background: rgba(255, 255, 255, 0.6);
        border: 1px solid #ccc;
        border-radius: 4px;
        padding: 2px 4px;
    }

</style>
<body>
    <script src="assets/static/js/initTheme.js"></script>
    <div id="app">
        <?= Flight::menu() ?>
        <div id="main">
            <div class="modal fade text-left" id="large" tabindex="-1" role="dialog" aria-labelledby="myModalLabel17" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg"
                    role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title" id="myModalLabel17">ajouter une infrastructure</h4>
                            <button type="button" class="close" data-bs-dismiss="modal"
                                aria-label="Close">
                                <i data-feather="x"></i>
                            </button>
                        </div>
                        <div class="modal-body">
                            <section id="multiple-column-form">
                                <div class="row match-height">
                                    <div class="col-12">
                                        <div class="card">
                                            <div class="card-content">
                                                <div class="card-body">
                                                    <form id="infraForm" class="form" method="get" action="<?= Flight::base() ?>/edition/add">
                                                        <div class="row">
                                                            <div class="col-md-6 col-12">
                                                                <div class="form-group">
                                                                    <label for="first-name-column">type</label>
                                                                    <select class="form-select" id="basicSelect" name="type_id">
                                                                        <?php foreach ($type as $x) { ?>
                                                                            <option value="<?= $x['id'] ?>"><?= $x['nom'] ?></option>
                                                                        <?php } ?>                                                                                                                
                                                                    </select>
                                                                </div>
                                                            </div>
                                                            <div class="col-md-6 col-12">
                                                                <div class="form-group">
                                                                    <label for="last-name-column">nom de l'infrastructure</label>
                                                                    <input type="text" id="last-name-column" class="form-control" placeholder="ex: hopital des enfants" name="nom">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-12 col-12">
                                                                <div class="form-group">
                                                                    <label for="company-column">descriptif</label>
                                                                    <div class="form-group with-title mb-3">
                                                                        <textarea placeholder="ex: achat d'ordinateur" class="form-control" id="exampleFormControlTextarea1" rows="3" name="descriptif"></textarea>
                                                                        <label>un lieu trés calme et paisible parfait pour se soigner !</label>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-12 d-flex justify-content-end">
                                                                <button type="submit" class="btn btn-primary me-1 mb-1">valider</button>
                                                                <button type="reset" class="btn btn-light-secondary me-1 mb-1">reinitialiser</button>
                                                            </div>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </section>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-light-secondary"
                                data-bs-dismiss="modal">
                                <i class="bx bx-x d-block d-sm-none"></i>
                                <span class="d-none d-sm-block">annuler</span>
                            </button>
                        </div>
                    </div>
                </div>
            </div>
            <div id="map"></div>
        </div>
    </div>
    <script src="<?= Flight::base() ?>/public/assets/static/js/components/dark.js"></script>
    <script src="<?= Flight::base() ?>/public/assets/extensions/perfect-scrollbar/perfect-scrollbar.min.js"></script>
    <script src="<?= Flight::base() ?>/public/assets/compiled/js/app.js"></script>

    <!-- modul js -->
    <script>const BASE_URL = "<?= Flight::base() ?>";</script>
    <script src="<?= Flight::base() ?>/public/js/map/map-home.js"></script>
    <script src="<?= Flight::base() ?>/public/js/map/edition.js"></script>

    <script src="<?= Flight::base() ?>/public/assets/extensions/flatpickr/flatpickr.min.js"></script>

    <!-- Leaflet JS & CSS -->
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
</body>
</html>